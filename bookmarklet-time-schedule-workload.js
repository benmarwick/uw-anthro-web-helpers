...
<div id="dashboard" style="display:none;">
    <div class="dashboard-header">
        ...
        <div class="dashboard-links">
            ...
            <a href="https://admin.artsci.washington.edu/curriculum/curriculum-planning-general-information" target="_blank">CAS Curriculum Policies ↗</a>
        </div>
    </div>
    <div id="current-quarter-view">
        <div class="filter-bar">
            ...
            <div class="filter-group" style="margin-left: 10px; border-left: 1px solid #ddd; padding-left: 15px;">
                <span style="font-size: 12px; font-style: italic; color: #555;">
                    <span style="opacity:0.7;">⚠️</span> <a href="https://admin.artsci.washington.edu/curriculum/curriculum-planning-general-information" target="_blank" style="color:#4b2e83;">CAS enrollment minimums</a>
                </span>
            </div>
        ...
        <div class="table-container" style="margin-top: 25px;">
            ...
            <table id="instructor-table" class="display" style="width:100%">
                <thead><tr>
                    ...
                    <th title="Total Student Credit Hours (SCH) assigned to this instructor. Includes checks against CAS Curriculum Guidelines for minimum class sizes. Underneath each total, you will see a detailed chronological formula. If co-taught, the load is divided equally.">Total SCH & Breakdown</th>
    ...
    newWin.document.write(htmlLayout);

    // Initialize Variables into New Window
    ...
    // Scraper logic kept isolated to prevent regex bugs during injection
    function runScraper() {
            ...
            try {
                ...
                function renderDashboard(){
                    ...
                    for (var b = 0; b < binCount; b++) {
                        xBins.push((b * binSize) + "-" + ((b * binSize) + binSize));
                    }
                    ...
                    var histogramTraces = activePrefixes.map(function(prefix) {
                        ...
                        return { x: xBins, y: yValues, type: "bar", name: prefix, marker: { color: colorBrewerSet2[prefix], line: { color: "#ffffff", width: 0.5 } }, text: hoverTexts, hoverinfo: "text" };
                    ...
                    Plotly.react("chart-instructor-sch", histogramTraces, {
                        title: "Instructor SCH Workload Distribution Histogram (" + ayDisplay + ")", barmode: "stack", xaxis: { title: "Annual Instructor Workload Bracket (SCH)" }, yaxis: { title: "Number of Instructors (by Discipline)" }, margin: { t: 50, l: 50, r: 20, b: 50 }, showlegend: true,
                        shapes: [{ type: 'line', xref: 'x', yref: 'paper', x0: avgPlotCoord, y0: 0, x1: avgPlotCoord, y1: 1, line: { color: 'rgb(16, 185, 129)', width: 2.5, dash: 'dash' } }, { type: 'line', xref: 'x', yref: 'paper', x0: targetPlotCoord, y0: 0, x1: targetPlotCoord, y1: 1, line: { color: 'rgb(75, 46, 131)', width: 2.5, dash: 'dot' } }],
                        annotations: [{ x: avgPlotCoord, y: 0.95, yref: 'paper', text: 'Average: ' + Math.round(avgSchPerInstructor) + ' SCH', showarrow: false, xanchor: 'right', yanchor: 'top', font: { color: 'rgb(16, 185, 129)', size: 10, weight: 'bold' } }, { x: targetPlotCoord, y: 0.90, yref: 'paper', text: 'Target: ' + schTarget + ' SCH', showarrow: false, xanchor: 'left', yanchor: 'top', font: { color: 'rgb(75, 46, 131)', size: 10, weight: 'bold' } }]
                    }, {responsive: true});
                    ...
                    for (var g = 0; g < 4; g++) {
                        ...
                        for (var s = 0; s < size; s++) {
                            if (currentIdx < N) { groups[g].push(sortedAsc[currentIdx]); currentIdx++; }
                        }
                    }
                    ...
                    var lorenzTrace = { x: labels, y: quartilePcts, type: "bar", marker: { color: "rgba(75, 46, 131, 0.8)", line: { color: "rgb(75, 46, 131)", width: 1.5 } }, text: quartilePcts.map(function(p) { return p.toFixed(1) + "%"; }), textposition: "auto", hovertext: quartileHoverTexts, hoverinfo: "text" };
                    ...
                    instructorSchList.forEach(function(inst) {
                        ...
                        var detailLines = inst.courses.map(function(c) {
                            ...
                            var qYrShort = qYearMatch ? qYearMatch[0].substring(2) : "";
                            var qDisplay = qAbbr + qYrShort;
                            
                            // CAS Enrollment Minimums Logic
                            var levelDigit = parseInt(c.number.charAt(0), 10);
                            var policyFlag = "";
                            if (levelDigit >= 1 && levelDigit <= 5) {
                                var minReq = (levelDigit <= 3) ? 10 : 5;
                                if (c.enrl < minReq) {
                                    policyFlag = " <span title='Below CAS minimum (" + minReq + " students)' style='opacity: 0.7; cursor: help; font-size: 11px;'>⚠️</span>";
                                }
                            }
                            
                            return "• " + qDisplay + " " + c.prefix + " " + c.number + " " + c.credits + " cr " + c.enrl + "/" + c.lim + " students" + policyFlag;
                        }).join("<br>");
                        ...
                        if (inst.courses.length === 1) {
                            ...
                            inst.courses.forEach(function(c) {
                                if (c.sch > maxLocalSch) { maxLocalSch = c.sch; maxSchCourse = c; }
                                var p = c.sch / totalSch;
                                if (p > 0) entropy -= p * Math.log(p);
                            });
                            var evenness = Math.log(inst.courses.length) > 0 ? (entropy / Math.log(inst.courses.length)) * 100 : 0;
                            var dependency = (maxLocalSch / totalSch) * 100;
                            balanceOrder = Math.round(evenness);
                            var balProfile = evenness >= 75 ? "Balanced" : (evenness >= 45 ? "Moderately Skewed" : "Highly Skewed");
                            var balEmoji = evenness >= 75 ? "🟢" : (evenness >= 45 ? "🟡" : "🔴");
                            var balColor = evenness >= 75 ? "color: rgb(4, 120, 87);" : (evenness >= 45 ? "color: rgb(180, 83, 9);" : "color: rgb(185, 28, 28); font-weight: bold;");
                            balanceCell = "<div style='font-size: 12px; line-height: 1.4;'><strong style='" + balColor + "'>" + balEmoji + " " + balProfile + " (" + Math.round(evenness) + "% Even)</strong><br><span style='color: #666; font-size: 10px;'>" + Math.round(dependency) + "% of SCH from " + (maxSchCourse ? maxSchCourse.prefix + " " + maxSchCourse.number : "") + "</span></div>";
                        }
                        ...
                        if (totalSch > schTarget * 1.5) {
                            profile = "Very High Load"; badgeColor = "background-color: rgb(254, 226, 226); color: rgb(185, 28, 28); border: 1px solid rgb(239, 68, 68); font-weight: bold;"; emoji = "🔴";
                        } else if (totalSch > schTarget * 1.1) {
                            profile = "High Workload"; badgeColor = "background-color: rgb(254, 243, 199); color: rgb(180, 83, 9); border: 1px solid rgb(245, 158, 11);"; emoji = "🟡";
                        } else if (totalSch < schTarget * 0.4) {
                            profile = "Under-utilized"; badgeColor = "background-color: rgb(243, 244, 246); color: rgb(75, 85, 99); border: 1px solid rgb(209, 213, 219);"; emoji = "⚪";
                        }
                        ...
                        tbodyHtml += "<tr><td><strong style='color:rgb(75, 46, 131);'>" + cleanName + "</strong></td><td style='text-align: center;' data-order='" + totalCredits + "'>" + creditCell + "</td><td style='text-align: center;' data-order='" + inst.courseCount + "'>" + inst.courseCount + "</td><td data-order='" + totalSch + "'>" + schCellContent + "</td><td data-order='" + balanceOrder + "'>" + balanceCell + "</td><td data-order='" + totalSch + "'>" + statusBadge + "</td></tr>";
                    });
                    
                    if (instTable) instTable.destroy();
                    $("#instructor-table tbody").html(tbodyHtml);
                    instTable = $("#instructor-table").DataTable({ pageLength: 15, order: [[3, "desc"]], language: {search: "Search Instructor:"} });
                    console.groupEnd();
                }
                renderDashboard();
                $(".prefix-filter, .gened-filter").on("change", renderDashboard);
                $("#bench-slider").on("input", function(){ schTarget = parseInt(this.value, 10); $("#bench-val").text(schTarget + " SCH"); renderDashboard(); });
                window.addEventListener("resize", function(){ Plotly.Plots.resize("chart-instructor-sch"); Plotly.Plots.resize("chart-lorenz-quantile"); });
                console.groupEnd();
            } catch (error) { console.error("Critical Stage 2 Scraper Error: ", error); }
        })();
    }

    // Convert the function to a string to inject it. This cleanly bypasses string escaping rules!
    newWin.document.write(`<script>(${runScraper.toString()})();<\/script>`);
